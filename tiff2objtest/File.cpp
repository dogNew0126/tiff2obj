...
public:
    bool open(const char* filename, OpenMode open_mode)
    {
        ...
        std::cout << "fopen(" << filename << ", " << omstr << ") = " << (fp != nullptr) << " / errno = " << err << std::endl;
    ...
    bool close()
    {
        ...
        if (m_fp != nullptr)
        {
            ...
            if (rc != 0)
            {
                std::cout << "fclose on " << m_filename << " failed with errno " << err << std::endl;
    ...
    size_t read(position_type from_offset, unsigned char* buffer, size_t size_of_buffer)
    {
        ...
        {
            std::cout << "huge read > INT_MAX bytes, won't do that" << std::endl;
    ...
    bool write(position_type to_offset, const unsigned char* data, size_t data_size)
    {
        ...
        {
            std::cout << "huge write > INT_MAX bytes, won't do that" << std::endl;
        ...
        if (rc != data_size)
        {
            std::cout << "unable to write " << data_size << " bytes into file " << m_filename << ", errno = " << err << std::endl;
            //TNTN_LOG_ERROR(
    ...
    void flush()
    {
        if (m_fp)
        {
            ...
            if (rc != 0)
            {
                std::cout << "fflush failed, errno = " << m_filename << ", " << err << std::endl;
...
private:
    bool seek_to_position(position_type position)
    {
        ...
        {
            std::cout << "unable to fseek correctly in file " << m_filename << ", errno = " << err << std::endl;
    ...
    size_t size_of_buffer)
{
    ...
    {
        std::cout << "unable to read more then SIZE_T_MAX bytes from MemoryFile" << std::endl;
...
bool MemoryFile::write(position_type to_offset_ui64, const unsigned char* data, size_t data_size)
{
    ...
    {
        std::cout << "unable to write more then SIZE_T_MAX bytes into MemoryFile" << std::endl;
    ...
    if (to_offset + data_size > m_data.size())
    {
        ...
        {
            std::cout << "unabled to resize MemoryFile to " << to_offset + data_size << ", std::bad_alloc" << std::endl;
    ...
    std::string& str)
{
    ...
    if (!f.is_good())
    {
        std::cout << "file-like object is not in a good state, cannot getline" << std::endl;
    ...
    std::string& str)
{
    if (!f)
    {
        std::cout << "cannot getline from file-like object that is NULL" << std::endl;
