...
BBox2D::BBox2D(glm::dvec3 a, glm::dvec3 b) : BBox2D(glm::dvec2(a.x, a.y), glm::dvec2(b.x, b.y)) {}

BBox2D::BBox2D(const Triangle& t) : BBox2D(glm::dvec2(t[0].x, t[0].y), glm::dvec2(t[1].x, t[1].y))
{
    add(glm::dvec2(t[2].x, t[2].y));
}
    ...
    glm::dvec2 l_dir)
{
    ...
    //z = mx + n
    const double d_p0p1 = glm::distance(glm::dvec2(p0.x, p0.y), glm::dvec2(p1.x, p1.y));
    const double m = (p1.z - p0.z) / d_p0p1;
    const double n = p0.z;

    const double d_p0c = glm::distance(glm::dvec2(p0.x, p0.y), glm::dvec2(c.x, c.y));
