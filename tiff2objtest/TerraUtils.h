...
inline void compute_plane(Plane& plane, dt_ptr t, const RasterFloat& raster)
{
...
{
public:
    TerraBaseMesh() : m_raster(std::make_unique<RasterFloat>()) {}

    void load_raster(std::unique_ptr<RasterFloat> raster);

protected:
    std::unique_ptr<RasterFloat> m_raster;
