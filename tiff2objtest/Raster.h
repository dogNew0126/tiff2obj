...
public:
    Raster() { NDVDefault<T>::set(m_noDataValue); }
    ...
    */
    ...
    void crop(const int cx, const int cy, const int cw, const int ch, Raster& dst_raster) const
    {
        ...
        if (max_x > m_width)
        {
            std::cout << "raster.crop - max x " << max_x << " larger than width " << m_width << std::endl;
            //TNTN_LOG_DEBUG("raster.crop - max x {} larger than width {}", max_x, m_width);
        }

        if (max_y > m_height)
        {
            std::cout << "raster.crop - max y " << max_y << " larger than height " << m_height << std::endl;
            //TNTN_LOG_DEBUG("raster.crop - max y {} larger than height {} ", max_y, m_height);
        }

        if (min_x < 0)
        {
            std::cout << "raster.crop - min x " << min_x << " smaller than zero" << std::endl;
            //TNTN_LOG_DEBUG("raster.crop - min x {} smaller than zero", min_x);
        }

        if (min_y < 0)
        {
            std::cout << "raster.crop - min y " << min_y << " smaller than zero" << std::endl;
    ...
    */
    ...
    {
        return isnan_helper<T>::isnan(a_value) || a_value == m_noDataValue;
    }
...
typedef Raster<float> RasterFloat;