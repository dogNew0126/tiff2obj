...
/** downsample image by an integer factor
    ...
    */
RasterFloat raster_tools::integer_downsample_mean(const RasterFloat& src, int win)
{
    int w = src.get_width();
...
RasterFloat raster_tools::convolution_filter(const RasterFloat& src,
    ...
    int size)
{
    ...
    float* pS = src.get_ptr(0);
    int s2 = size / 2;

    // not optimsed yet!
    for (int r = s2; r < h - s2; r++)
    {
        float* pD = dst.get_ptr(r);
...
RasterFloat raster_tools::max_filter(const RasterFloat& src,
    int size,
    double pos,
    double factor)
{
...
void raster_tools::flip_data_x(RasterFloat& raster)
{
...
void raster_tools::flip_data_y(RasterFloat& raster)
{
    int row = 0;
    const int height = raster.get_height();
    const int width = raster.get_width();
...
void raster_tools::find_minmax(const RasterFloat& raster, double& min_val, double& max_val)
{
    if (raster.empty())
    {
        return;
    ...
    float min = raster.value(0, 0);
    float max = raster.value(0, 0);
...
/**
    ...
    */
BBox3D raster_tools::get_bounding_box3d(const RasterFloat& raster)
{
...
static inline double safe_get_pixel(
    const RasterFloat& src, const int64_t w, const int64_t h, const int64_t r, const int64_t c)
{
    return r >= 0 && r < h && c >= 0 && c < w ? src.value(r, c) : NAN;
}

static double subsample_raster_3x3(const RasterFloat& src,
...
double raster_tools::sample_nearest_valid_avg(const RasterFloat& src,
