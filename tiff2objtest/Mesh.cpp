...
{
    assert((!has_triangles() || !has_decomposed()) ||
...
void Mesh::generate_triangles()
{
    ...
    std::cout << "generate triangels..." << std::endl;
...
void Mesh::generate_decomposed()
{
    ...
    std::cout <<" generate decomposed... " << std::endl;
...
{
    assert(this->has_triangles());
    assert(other.has_decomposed());
...
bool Mesh::check_for_holes_in_square_mesh() const
{
    ...
    if (is_square())
    {
        std::cout << "mesh is square - checking for holes" << std::endl;
        ...
        {
            std::cout << "mesh has holes. area of bounding box " << area_bb
					  << " area of all triangles combined " << area_tri_sum << std::endl;
            //TNTN_LOG_DEBUG(
        ...
        {
            std::cout << "mesh has no holes" << std::endl;
    ...
    {
        std::cout << "mesh is not square - can not check for holes" << std::endl;
...
{
    std::cout << "checking mesh consistency / TIN properties..." << std::endl;
    ...
    //O(n) checks first
    {
        ...
        for (const Face& f : m_faces)
        {
            ...
            {
                std::cout << "mesh is NOT a regular/propper TIN, some face indices are not valid" << std::endl;
                //TNTN_LOG_DEBUG(
            ...
            {
                std::cout << "mesh is NOT a regular/propper TIN, some faces have collapsed corner points" << std::endl;
                //TNTN_LOG_DEBUG(
            ...
            {
                std::cout << "triangle is NOT a regular/propper TIN, some faces are not oriented upwards" << std::endl;
                //TNTN_LOG_DEBUG(
        ...
        for (const char is_used : vertex_used)
        {
            if (!is_used)
            {
                std::cout << "mesh is NOT a regular/propper TIN, some vertices are not referenced by a face" << std::endl;
                //TNTN_LOG_DEBUG(
    ...
    //no duplicate vertices (this is O(n*log(n))
    {
        ...
        {
            std::cout << "mesh is NOT a regular/propper TIN, there are duplicate vertices" << std::endl;
    ...
    for (size_t fi = 0; fi < m_faces.size(); fi++)
    {
        ...
        {
            std::cout << "mesh is NOT a regular/propper TIN, some triangles are overlapping" << std::endl;
    ...
    //https://en.wikipedia.org/wiki/Euler_characteristic
    {
        ...
        if (chi != 2) {
            std::cout << "mesh is NOT a regular/propper TIN, some triangles are missing (euler charachteristics violated)" << std::endl;
    ...
    std::cout << "mesh is a regular/propper TIN" << std::endl;
